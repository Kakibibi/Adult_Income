---
title: UCI Adult Income dataset- data cleaning and processing
author:
...
Good data preprocessing is crucial for reliable and interpretable results in machine learning and analytics workflows. Here, we address common data issues such as **missing values, duplicates, and inconsistent categorical labels** while creating derived features to improve downstream analysi 
...
## Read in the data
We load th ***Adult Income Dataset *** as a CSV file.andKey consideration here are:
...
## Data Cleaning
   ## 1. Assign proper column names to the colums
...
**Table 9: Mapping from `native_country` to `native_region`*native countrycation   native regionn Level     |
|:-----------------|:--------------------|
...
adult_df.loc[:,'age_group'] = pd.cut(adult_df['age'], bins=bins, labels=labels,right=True,include_lowest=False)
adult_df
```
...
```{python}
adult_df.drop(columns=['education', 'native_country', 'occupation'], inplace=True, errors=('ignore')
)
```
